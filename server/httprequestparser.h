...
private:
    ...
    ParseResult consume(Request &req, const char *begin, const char *end)  //while循环判断资源标识符
    {
        while( begin != end )
        {
            char input = *begin++;
            ...
            case Post:
                --contentSize;
                std::cout<<"get a input"<< input <<std::endl;
    ...
    inline bool isChar(int c)      //内联函数检测是不是字符
    {
        return c >= 0 && c <= 127;
    }

    // Check if a byte is an HTTP control character.
    inline bool isControl(int c)   //是不是控制字符
    {
        return (c >= 0 && c <= 31) || (c == 127);
    }

    // Check if a byte is defined as an HTTP special character.
    inline bool isSpecial(int c)   //是不是特殊字符
    {
        switch (c)
        {
